...
func (d *Devops) GroupByTime(qi query.Query, nHosts, numMetrics int, timeRange time.Duration) {
	...
	if nHosts == 1 {
		panicIfErr(err)
		sql = fmt.Sprintf(`SELECT time_bucket(k_timestamp, '60s') as k_timestamp, %s FROM %s.cpu WHERE hostname='%s' AND k_timestamp >= '%d' AND k_timestamp < '%d' GROUP BY time_bucket(k_timestamp, '60s') ORDER BY time_bucket(k_timestamp, '60s')`,
