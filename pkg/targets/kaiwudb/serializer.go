...
func (s *Serializer) Serialize(p *data.Point, w io.Writer) error {
	...
	if !exist {
		...
		columnsForInsertMetrics := "(k_timestamp:" + strings.Join(fieldKeys, ":") + ":hostname)"
		fmt.Fprintf(w, "%c,%s,%s,create table %s (k_timestamp timestamp not null%s) tags (%s) primary tags (hostname)\n",
			CreateTable, superTable, columnsForInsertMetrics, superTable, columnsStr, tagsStr)
		// fmt.Fprintf(w, "%c,%s,(k_timestamp timestamp not null%s) tags (%s) primary tags(hostname)\n", CreateTemplateTable, superTable, columnsStr, tagsStr)
		s.tmpBuf.Reset()
		table := &Table{
			columns:    map[string]struct{}{},
			tags:       map[string]struct{}{},
